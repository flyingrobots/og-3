...
Two transport paths deliver the same final document to the same endpoint observer.
Along the first path, the object carries a signed edit receipt and a freshness
frontier at which the signature and policy context can be checked. Along the
second path, the object carries only the final text. To an endpoint observer
whose task is merely to read the document, the two arrivals agree: the visible
state is the same. To an observer whose task is to audit authorship, they do not
agree. The claim ``the document contains text $X$'' may be supported along both
paths, while the claim ``Alice authorised edit $e$'' is supported along only the
path that carried admissible provenance support.

This is the obstruction studied in OG-III. Endpoint agreement is not path
neutrality. Even after a source observer, a target observer, and a target visible
state are fixed, the justified status of a claim may depend on which witnesses,
receipts, authority metadata, freshness frontiers, and disclosure constraints
survived the route by which the object arrived. The underlying event may be
invariant; the justified claim-status need not be.
...
observer signatures~\cite{ross_og_i_2026}. Its central lesson was that observers
are not interchangeable views over a single neutral state: what an observer can
support depends on what it projects, retains, resolves, and is structurally able
to distinguish. OG-II developed the distributed case. It showed that replicas
may converge in visible state while their observer surfaces remain different,
because witnessed updates, frontiers, suffix transport, policy closure,
provenance, and obstruction structure may be retained or erased differently
along the way~\cite{ross_og_ii_2026}. OG-III studies the next obstruction:
state convergence and endpoint agreement do not determine claim support.

The point is not that facts become arbitrary once they are transported. The
point is sharper and more finite. A claim made for a purpose imposes a support
obligation. A provenance claim, an absence claim, an authority claim, a freshness
claim, and a state claim do not demand the same evidence. A transport path may
carry that support directly, compress it into a receipt, replace it with a
cryptographic proof, bind it for future opening, declare it redacted, mark it
authority-blocked, or lose it entirely. The endpoint observer's responsible
status for the claim is bounded by that transported support.

The basic accounting objects of OG-III are therefore claim-relative and
path-relative. For a claim $\varphi$, a purpose $P$, and a support context
$\mathrm{ctx}$, the paper uses
\[
  \mathrm{Obl}_{P}(\varphi,\mathrm{ctx})
\]
for the support obligation imposed by the claim,
\[
  \mathrm{Carry}_{\gamma}(\varphi,\mathrm{ctx})
\]
for the support actually carried, compressed, opened, certified, blocked,
redacted, or declared lost along an observer path $\gamma$, and
\[
  \mathrm{Val}_{\gamma}^{P,\mathrm{ctx}}(\varphi)
\]
for the justified status of $\varphi$ after transport along that path. When a
path preserves enough visible structure to make a claim discussable while
dropping the support needed to justify the reported status of that claim, the
path creates witness debt. Witness debt is structural: it is not yet dishonesty.
Dishonesty enters only when an observer reports as if that debt were discharged.

This paper keeps the geometry finite, discrete, and witness-based. Observer
paths are finite chains of translators between structured observers. A
translator is not assumed to preserve all coordinates; it preserves only those
coordinates, witnesses, receipts, authority conditions, and freshness
constraints declared for the task. A path is therefore not merely a route along
which state moves. It is a support transformer. It may preserve support,
compress support, certify support, redact support, block support, launder
support, or create support debt.

This shift from endpoint comparison to claim transport is the main contribution
of OG-III. OG-I asks what an observer can see and distinguish. OG-II asks how
distributed observers may converge in state without converging as observers.
OG-III asks which claims survive transport between observers, which become
underdetermined, which are refuted, which are unverifiable, which are
authority-blocked, and which are merely overreported. In short:
observer-relative truth is governed by the path-sensitive transport of support
obligations.

Cryptographic witnesses play a central role because they make several observer
...
claim authority-blocked rather than false. These primitives are not treated here
as protocol novelties. They are finite witness surfaces: mechanisms that
separate visibility, retained support, verification, revelation, authority, and
freshness.

This separation also prevents a common mistake. Raw cryptographic verification
does not by itself imply observer admissibility. A proof may verify as a
cryptographic object while failing to satisfy the support obligation of a
particular observer, purpose, authority domain, disclosure posture, or freshness
frontier. Thus OG-III distinguishes
\[
  \mathrm{Verify}(\mathrm{CryptSupp},\mathrm{ctx}) = \mathrm{accept}
\]
from
\[
  \mathrm{Admit}_{S}(\mathrm{CryptSupp},\varphi,P,\mathrm{ctx})
  = \mathrm{accept}.
\]
The first is a verification relation over a cryptographic witness surface and
its bound context. The second is an observer-relative admission judgement. The
gap between them is not a bug in the formalism; it is one of the phenomena the
formalism is designed to expose.

The paper proves five main finite separation results. First, witness
sensitivity: there are claims whose status is not determined by projection
alone but is determined by preserved support. Second, path-dependent support
transport: two paths with the same source, target, and endpoint visible state
can induce different justified statuses for the same claim. Third, nonzero
witness holonomy: a closed observer path may return visible state to its
starting value while changing provenance, authority, witness, or verification
support. Fourth, validity/admissibility separation: cryptographic acceptance
does not imply observer-relative admissibility for a declared purpose or
authority domain. Fifth, support-soundness failure: an observer can report a
claim as supported even though the path-carried support does not satisfy the
claim's support obligation. Each result is witnessed by a finite observer graph
and a finite support table; no smooth manifold, infinitesimal connection, or
global metric is assumed.

The central slogan is therefore:
\[
  \text{same endpoint state} \;\not\Rightarrow\; \text{same transported support}.
\]
For state-reading tasks, two arrivals may be equivalent. For audit,
non-revocation, authorship, intent, safety, liveness, or authority tasks, they
may diverge. Observer equivalence is purpose-indexed, and claim robustness is a
property of a class of paths, not merely of two endpoint observers.

OG-III remains structural. It analyzes false, unsupported, stale, blocked,
laundered, and overstrong reports as support-status mismatches. It does not yet
