...
Use the signed-edit witness family of \cref{app:witness-library}. The certified
path carries the signature, identity binding, policy epoch, and freshness
frontier required by the authorship-audit obligation. The raw path carries only
the final text. Both paths project to the same endpoint document text, while
the certified path satisfies the authorship obligation and the raw path leaves
that obligation unmet. The detailed ledger check is given in
\cref{app:proofs}.
\end{proof}
...
set
\[
  \Obl_{P_{\mathrm{read}}}(\phi_{\mathrm{text}},\ctx)=\{s_X\}.
\]
...
\[
  x_{\mathrm{cert}}=(X,\{s_X,\sigma_A,i_A,p_\alpha,f_F\}),
  \qquad
  x_{\mathrm{raw}}=(X,\{s_X\}).
\]
...
For these examples, $\Sat(C,O)=\mathrm{yes}$ exactly when $O\subseteq C$ or
when $C$ contains an admitted compression receipt for $O$. If $C$ carries some
support relevant to $O$ without discharging all required atoms,
$\Sat(C,O)=\mathrm{partial}$. If $C$ carries no required atom and no admitted
compression receipt, $\Sat(C,O)=\mathrm{no}$.
...
\[
  ...
  \{s_X\}.
\]
Both paths deliver document text $X$ to $S_1$, so
\[
...
$S_0\to S_1$ carries $\{s_X\}$ for the authorship claim. The mediated path
...
support:
\[
  T_r(\{s_X,\sigma_A,i_A,p_\alpha,f_F\})=\{s_X,p_\alpha,f_F\}.
\]
Assume $T_r$ preserves already-issued receipts:
\[
  T_r(\{s_X,r_{\mathrm{auth}}\})=\{s_X,r_{\mathrm{auth}}\}.
\]
Then
\[
...
\]
Let each translator preserve $s_X$. Let $T_2$ replace the full authorship
support with a text-only summary:
\[
  T_2(\{s_X,\sigma_A,i_A,p_\alpha,f_F\})=\{s_X\}.
\]
Let $T_3$ and $T_4$ preserve this text-only support. Then the path closes on
the visible state coordinate and fails to close on the authorship witness
coordinate.

\paragraph{A.8 Witness-debt path.}
Use the raw path $\gamma_{\mathrm{raw}}$ and the authorship claim. The carried
support is $\{s_X\}$ and the obligation is
...
bundle contains every atom in
\[
  \Obl_{P_{\mathrm{auth}}}(\phi_{\mathrm{auth}},\ctx)
  =
  \{\sigma_A,i_A,p_\alpha,f_F\}.
\]
Thus $\Sat=\mathrm{yes}$ and the status is $\supported$. The raw support bundle
is $\{s_X\}$, which contains no authorship-obligation atom and no admitted
compression receipt. Thus $\Sat=\mathrm{no}$. With no carried refutation, the
status is $\underdetermined$. Equal projection therefore fails to determine
equal support status.

\paragraph{D.2 Mediated Support Translation (\cref{prop:mediated-support-translation}).}
Use A.4. On the direct path, the endpoint carries $\{s_X\}$ for
...
Its carried ledger is $\{s_X,r_{\mathrm{auth}}\}$. The composite $T_c\circ T_r$
...
ledger is $\{s_X,p_\alpha,f_F\}$. The ledgers differ as finite sets, so support
transport is order-sensitive.
...
have source $S_0$, target $S_1$, and endpoint text $X$. Therefore the endpoint
visible-state equality required by the theorem holds. The certified path
carries the complete authorship obligation and is supported. The raw path
carries endpoint text only and is underdetermined for the authorship claim.
Hence the paths agree on endpoint visible state and disagree on justified
claim-status.
...
$\{\sigma_A,i_A,p_\alpha,f_F\}$ and returns as the empty authorship component
because $T_2$ replaced it with text-only support. Since the missing component
is exactly the finite obligation for $\phi_{\mathrm{auth}}$,
\[
...
Use A.8. The raw path carries $\{s_X\}$ and the authorship obligation is
$\{\sigma_A,i_A,p_\alpha,f_F\}$. Therefore
\[
